...
packages <- c("ggplot2", "dplyr", "tidyr", "tidyverse", "DataExplorer", "ggridges", "hrbrthemes", "viridisLite", "viridis", "readr", "packcircles", "ggiraph", "Hmisc", "kableExtra", "lubridate", "plotly", "data.table", "formattable", "visNetwork", "knitr", "sjmisc", "mitre", "rworldmap", "stringr", "maps")
ipak(packages)
...
Primero de todo, extraemos todas las técnicas que encontramos en el fichero *incidents* y procedemos a investigar cuales de estas técnicas se encuentran en ATT&CK y en CAR (Standard que hace referencia a técnicas de ATT&CK).  
...
qplot(x=id, data=temp,fill=as.factor(id)) + labs(x = "Técnicas", y = "Count") + theme( axis.title.x = element_text(color = "black", size = 12, face = "bold"), axis.title.y = element_text(color = "black", size = 14, face = "bold")) + scale_fill_manual(values=viridis(5, alpha = 1, begin = 0, end = 0.5, option = "D")) + theme_ipsum() + labs(fill = 'ids')
```
...
qplot(x=id, data=temp, fill=as.factor(id)) + labs(x = "Tecnicas", y = "Count") + theme( axis.title.x = element_text(color = "black", size = 14, face = "bold"), axis.title.y = element_text(color = "black", size = 14, face = "bold")) + scale_fill_manual(values=viridis(16, alpha = 1, begin = 0, end = 1, option = "D")) + theme_ipsum() + labs(fill = 'ids') + theme(axis.text.x = element_text(angle = 90))
```
...
Podemos observar que con el estudio sin las técnicas deprecadas cubrimos un 66.7% de los ataques de modo que se considera correcto seguir únicamente con estos. Cabe decir que, en caso de querer mejor la exactitud se podrían estudiar las técnicas *T1086* y *T1192* cubriendo, así, el 92% de los incidentes del estudio.  
...
) + scale_fill_manual(values=viridis(20, alpha = 1, begin = 0, end = 1, option = "D")) + scale_color_manual(values=viridis(20, alpha = 1, begin = 0, end = 1, option = "D"))+ theme_ipsum() + theme(axis.text.x = element_text(angle = 90)) + ylim(1,7) +  geom_segment( aes(x=Var1, xend=Var1, y=1, yend=Freq)) + labs(fill = 'groups', color='groups')
```
...
#S
mygCompletaGroups <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")%>%
...
#### Caso simplificado: T1072 y T1078
...
tempNodes1Simplificado$color <- viridis(length(tempNodes1Simplificado$color), alpha=0.9, begin=0.05, end=0.95)
...
mygSimplificadoGroups <- visNetwork::visNetwork(nodes = tempNodesSimplificado, edges = tempEdgesSimplificado, height = "500px", width = "100%")
visEdges(mygSimplificadoGroups, color="lightgrey", label = "")
```

# ¿Cuál fue el objetivo?

YO CREO QUE AQUI TENEMOS QUE PONER QUE HACE CADA TÉCNICA(?)

# ¿Cómo realizaron los ataques?

Para estudiar como se realizaron estos ataques se estudiarán las tácticas que fueron llevadas a cabo y los softwares usados. 
...
mygCompletoTactics <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
mygCompletoTactics
```
...
mygSimplificadoTactics <- visNetwork::visNetwork(nodes = tempNodesSimplificado, edges = tempEdgesSimplificado, height = "500px", width = "100%")
...
mygCompletoSoft <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
mygCompletoSoft
...
mygSimplificadoSoft <- visNetwork::visNetwork(nodes = tempNodesSimplificado, edges = tempEdgesSimplificado, height = "500px", width = "100%")
mygSimplificadoSoft
...
```{r mitigations attack, echo=FALSE, message=FALSE, warning=FALSE}
...
mygCompletoMitigations <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygSimplificadoMitigations <- visNetwork::visNetwork(nodes = tempNodesSimplificado, edges = tempEdgesSimplificado, height = "500px", width = "100%")
...
mygCompletoShieldDTE <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygSimplificadoShieldDTE <- visNetwork::visNetwork(nodes = tempNodesSimplificado, edges = tempEdgesSimplificado, height = "600px", width = "100%")
...
mygCompletoShieldDos <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygSimplificadoShieldDOS <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygCompletoShieldDPR <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygSimplificadoShieldDPR <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygCompletoShieldDUC <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
mygCompletoShieldDUC
```

#### Simplificado

```{r shiled:DUC:Simpl, echo=FALSE, message=FALSE, warning=FALSE, echo=FALSE}
#Ahora con el caso simplificado: DTEs (son 4) relacionados con las tecnicas T1072 y T1078
tempNodes <- unique(plyr::ldply(
    extracShieldSimplificado$from, function(x) mitre::getNodeNeighbors(node = x, mitrenet = mitre.data$standards$shield$shieldnet)[["nodes"]]
))
...
mygSimplificadoShieldDUC <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygCompletoShieldDTA <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
mygSimplificadoShieldDTA <- visNetwork::visNetwork(nodes = tempNodes, edges = tempEdges, height = "500px", width = "100%")
...
# Análisis completo sobre la técnica más usada: ***T1072***
