...
tecCar <- ids[which(ids %in% raw_car$carnet$edges$to)] #Tecnicas encontradas en car 5

tecAttack <- ids[which(ids %in% raw_attck$techniques$mitreid)] #Tecnicas encontradas en attack 16

tecAll <- ids[which(ids %in% mitre.data$mitrenet$edges$from)] #Tecnicas encontradas en toda la red de mitre 16

#Referencias a Attack -------------------------------------------------------
...
groupsAttackDf <- groupsAttack[which(groupsAttack$Freq > 2),]
length(groupsAttackDf[[1]])
qplot(x=Var1, y=Freq, data=groupsAttackDf)
...
#Continuamos etcnicas de shield
...
rowsDTEShieldDescrip %>% kbl() %>% kable_styling()
...
length(extracShieldDOS)[[1]]
length(unique(extracShieldDOS$to))
extracShieldDPR <- filter(extracShield, startsWith(as.character(extracShield$to), "DPR"))
length(extracShieldDPR[[1]])
length(unique(extracShieldDPR$to))
extracShieldDUC <- filter(extracShield, startsWith(as.character(extracShield$to), "DUC"))
length(extracShieldDUC[[1]])
length(unique(extracShieldDUC$to))
...
length(unique(extracShieldDTA$from))

#Continuamos por Software

sofwAttack <- filter(dfToAttack, startsWith(as.character(dfToAttack$from), "S"))
sofwAttack <- sort(table(sofwAttack$from))

mAttack <- filter(dfToAttack, startsWith(as.character(dfToAttack$from), "M"))
mAttack <- sort(table(mAttack$from))
...
table(k$to) #lo mostramos más adelante
...
groups <- dplyr::select(groups, mitreid, name, description)
